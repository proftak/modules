...
Based on the discussion of how values are represented by numbers in [module 0282](../0282), a base-2 number with digits 0 to ${w-1}$ (therefore a total of $w$ digits, $w$ is an integer that is at least 1) can represent values from 0 to $2^{w}-1$
...
In fact, it is true that for all $w>1$, the binary representation of $2^w-1$ consists of $w$ contiguous 1's! In the [binary subtraction module](../0283), we learn that $t_{i+1}=b(x_i,y_i)+b(q_i,k_i)=!x_iy_i + !(x_i \oplus y_i)t_i$. If $x_i$ is guaranteed a 1, and the initial $t_0=0$, then we can prove (using proof by induction) that $t_i=0$ for the entire $t$ row.
...
From a [previous module discussion how a value is represented by a number](../0282), we know that $1101_2$ represents the value of 13. But now we claim that the same bit pattern (binary number) also represents -3. So which one is it?
